...
export function registerSaveContainersTool(server: McpServer) {
	server.tool(
		"save_containers",
		"Save a container in an Agility instance.",
		{
		...
		async ({ instanceGuid }: { instanceGuid: string }, extra: any) => {
			...
			// 	const category = container.contentViewCategoryName || "Uncategorized";
			// 	if (!acc[category]) {
			// 		acc[category] = [];
			// 	}
			...
			// 	containersInCategory.sort((a, b) => (`${a.contentViewName}`.localeCompare(`${b.contentViewName}`)));
			// 	return `### ${category}\n` + containersInCategory.map(container => `- ${container.contentViewName} (${container.contentViewID})`).join("\n");
			// }).join("\n\n");
			...
			return {
				content: [{ type: "text", text: `This tool is not yet implemented. Please use the saveContainer tool to save a container.` }],
